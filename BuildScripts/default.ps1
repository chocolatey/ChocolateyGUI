...
Task -Name __InstallReSharperCommandLineTools -Depends __InstallChocolatey -Description $private -Action {
	...
	try {
		...
		if (-not (Test-Path $inspectCodeExe)) {
			exec {
				Invoke-Expression "$script:chocolateyCommand install JetBrains.ReSharper.CommandLineTools -Pre -y -source https://www.nuget.org/api/v2/";
			}
		} else {
...
Task -Name __UpdateReSharperCommandLineTools -Description $private -Action {
	try {
		...
		exec {
			Invoke-Expression "$script:chocolateyCommand upgrade JetBrains.ReSharper.CommandLineTools -Pre -y -source https://www.nuget.org/api/v2/";
		}
